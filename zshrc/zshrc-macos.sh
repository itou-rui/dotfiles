...
SCRIPT_PATH="$HOME/.config/scripts/macos/mac/400-autoPushGithub.sh"

# NOTE: If you modify the StartInterval below, make sure to also change it in
# the ~/.config/scripts/macos/mac/400-autoPushGithub.sh script
...
# the ~/.config/tmux/tools/linkarzu/tmuxKillSessions.sh script
#
# 1 hour = 3600 s
INTERVAL_SEC=7200
...
SCRIPT_PATH="$HOME/.config/tmux/tools/linkarzu/$PLIST_ID.sh"
...
# This is set in ~/.config/colorscheme/colorscheme-vars.sh
~/.config/zshrc/colorscheme-set.sh "$colorscheme_profile"
...
if command -v starship &>/dev/null; then
  type starship_zle-keymap-select >/dev/null ||
    {
      export STARSHIP_CONFIG=$HOME/.config/starship-config/active-config.toml
...
if [ -d "$HOME/.homebrew/opt/mysql-client/bin" ]; then
  export PATH="$HOME/.homebrew/opt/mysql-client/bin:$PATH"
fi
